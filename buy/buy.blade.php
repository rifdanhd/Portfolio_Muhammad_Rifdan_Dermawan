...
@push('styles')
    ...
    border:2px dashed rgba(26,20,69,.15);
    border-radius:10px; padding:14px 16px;
    cursor:pointer; transition:border-color .2s, background .2s;
    background:#fdfaf4;
}
.bukti-upload-area:hover { border-color:#c4a47c; background:#fff8ea; }
.bukti-upload-icon {
    width:36px; height:36px; flex-shrink:0; border-radius:8px;
    background:rgba(196,164,124,.15); color:#c4a47c;
    display:flex; align-items:center; justify-content:center;
}
.bukti-upload-text strong { display:block; font-size:12.5px; color:#1a1445; }
.bukti-upload-text span { font-size:10.5px; color:rgba(26,20,69,.5); }

.bukti-preview-card {
    display:flex; align-items:center; gap:10px;
    border:1px solid rgba(26,20,69,.1); border-radius:10px;
    padding:8px 10px; background:#fff;
}
.bukti-preview-card img {
    width:44px; height:44px; object-fit:cover; border-radius:6px; flex-shrink:0;
}
.bukti-preview-info { display:flex; flex-direction:column; gap:2px; flex:1; min-width:0; }
.bukti-preview-info span {
    font-size:11px; color:#1a1445; font-weight:600;
    white-space:nowrap; overflow:hidden; text-overflow:ellipsis;
}
.bukti-preview-info button {
    align-self:flex-start; background:none; border:none; color:#c4a47c;
...
.bukti-status.success { color:#2d9f6a; }
.bukti-status.error   { color:#e05252; }
.bukti-status.loading { color:rgba(26,20,69,.5); }

  /* Push AI capsule higher on mobile so it does not block the bottom sticky ticket summary / WhatsApp order button */
        ...
        * {
        ...
        .tkt-hero {
            ...
            align-items: center;
            overflow: visible;
            padding: 120px clamp(1.5rem, 5vw, 4rem) 20px;
        }
        ...
        .tkt-hero-bg {
            ...
            background:
                ...
                url('{{ asset('img/Angklungmasal.webp') }}');
        ...
        .tkt-event-poster {
            ...
            z-index: 10;
            flex-shrink: 0;
            align-self: flex-end;            /* docking ke bawah hero */
            margin-bottom: -90px;            /* overflow keluar bawah hero */
        ...
        .tkt-event-poster img {
            width: 100%;
            height: 100%;
            object-fit: contain;
            object-position: bottom;
            border-radius: 16px;
            filter: drop-shadow(0 32px 48px rgba(0, 0, 0, 0.6)) drop-shadow(0 6px 16px rgba(0,0,0,0.3));
            transition: transform .5s cubic-bezier(.25,.8,.25,1);
        }

        .tkt-event-poster img:hover {
        ...
        .breadcrumb {
            ...
            padding-top: 104px;              /* kompensasi poster overflow */
    ...
    flex-shrink: 0;
        ...
        .paym-opt:hover { border-color: var(--gold); background: var(--gold-lt); }
        .paym-opt input { accent-color: #1a1445; }
        .qris-box {
            display: none;
            text-align: center;
            margin-top: 10px;
        ...
        .qris-box img { width: 170px; margin: 0 auto 10px; display: block; border-radius: 8px; }
        .qris-box input[type=file] { font-size: 11px; }
...
@media(max-width:768px) {
    ...
    .tkt-event-poster {
        ...
        transform: none !important;
        margin-bottom: 0 !important;    /* reset overflow desktop */
    ...
    /* --- Breadcrumb --- */
    .breadcrumb { padding: 10px .9rem !important; padding-top: 10px !important; }
...
@media(max-width:400px) {
    .tkt-hero { height: auto !important; min-height: 180px !important; }
...
{{-- HERO --}}
<div class="tkt-hero">
    <div class="tkt-hero-bg-wrap">
        <div class="tkt-hero-bg"></div>
    </div>

    <div class="tkt-hero-inner">
        @if($event->banner_image)
            {{-- poster rendered AFTER hero-content so it stays on the right --}}
        @endif
        ...
        <div class="hero-content">
            ...
            <div class="hero-details">
                <div class="hero-detail-item">
                    <svg class="h-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
                        <path stroke-linecap="round" stroke-linejoin="round" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/>
                    </svg>
                    <span>{{ $event->location ?? 'Saung Angklung Udjo, Bandung' }}</span>
                </div>
                <div class="hero-detail-item">
                    <svg class="h-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                    ...
                    <span>
                        @if(isset($event->event_date_start))
                            ...
                            @if(isset($event->event_date_end) && $event->event_date_start != $event->event_date_end)
                ...
                <div class="hero-detail-item">
                    ...
                    <span>{{ $event->category ?? 'Pertunjukan Seni • Budaya • Alat Musik Bambu' }}</span>
                </div>
            </div>
        </div>

        @if($event->banner_image)
            <div class="tkt-event-poster">
                <img
                ...
                <div class="sum-body-wrapper" id="sum-body-wrapper">
                    ...
                    <div class="field" style="margin-bottom:0">
                        ...
                        <div style="display:flex;flex-direction:column;gap:8px;margin-top:8px">
                            <label class="paym-opt">
                                <input type="radio" name="pm" value="whatsapp" checked onchange="setPaymentMethod(this.value)">
                                Konfirmasi via WhatsApp (bayar di tempat)
                            </label>
                            <label class="paym-opt">
                                <input type="radio" name="pm" value="qris_static" onchange="setPaymentMethod(this.value)">
                                Bayar Sekarang via QRIS
                            </label>
                        </div>

                        <div id="qris-box" class="qris-box">
                            <img src="{{ asset('Qris/Qris.jpeg') }}" alt="QRIS">
                            <p style="font-size:11px;color:var(--gray-text);margin-bottom:10px">
                                Scan QR di atas menggunakan aplikasi e-wallet / m-banking, lalu upload bukti transfer.
                            </p>
                            <label for="bukti-file" class="bukti-upload-area" id="bukti-upload-area">
    <div class="bukti-upload-icon">
        <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            <path d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/>
        </svg>
    </div>
    <div class="bukti-upload-text">
        <strong>Klik untuk upload bukti transfer</strong>
        <span>JPG / PNG — maks. 2 MB</span>
    </div>
</label>
<input type="file" id="bukti-file" accept="image/*" style="display:none">

<div id="bukti-preview-wrap" style="display:none;margin-top:10px;">
    <div class="bukti-preview-card">
        <img id="bukti-preview-img" src="" alt="Preview bukti transfer">
        <div class="bukti-preview-info">
            <span id="bukti-filename">—</span>
            <button type="button" onclick="resetBuktiUpload()">Ganti foto</button>
        </div>
    </div>
</div>

<div id="bukti-status" class="bukti-status"></div>
                        </div>
                    </div>
                    @endif
...
function setPaymentMethod(v) {
    ...
    if (box) box.style.display = (v === 'qris_static') ? 'block' : 'none';

    const label = document.querySelector('#btn-pay .btn-label');
    if (label) {
        label.textContent = (v === 'qris_static')
            ? 'Konfirmasi Pembayaran QRIS'
            : 'Pesan Via WhatsApp';
    }
}

document.getElementById('bukti-file')?.addEventListener('change', async function () {
    const file = this.files[0];
    if (!file) return;

    const status      = document.getElementById('bukti-status');
    const uploadArea   = document.getElementById('bukti-upload-area');
    const previewWrap  = document.getElementById('bukti-preview-wrap');
    const previewImg   = document.getElementById('bukti-preview-img');
    const filenameEl   = document.getElementById('bukti-filename');

    // Tampilkan preview + sembunyikan area upload
    ...
    status.className   = 'bukti-status loading';
    ...
    try {
        const res = await fetch('{{ route('booking.upload-bukti') }}', {
            method: 'POST',
        ...
        if (data.url) {
            uploadedBuktiUrl    = data.url;
            status.className   = 'bukti-status success';
            status.textContent = '✅ Bukti berhasil diunggah';
        } else {
            status.className   = 'bukti-status error';
...
function resetBuktiUpload() {
    ...
    document.getElementById('bukti-preview-wrap').style.display = 'none';
    document.getElementById('bukti-status').textContent = '';
    document.getElementById('bukti-status').className   = 'bukti-status';
...
let userChangedPhoneCode = false;
document.getElementById('f-country')?.addEventListener('change', function () {
    if (!userChangedPhoneCode) {
        ...
        if (code) {
            ...
            /* ── TIKET: dinamis dari event yang dipilih (ticket_events / ticket_event_types) ── */
            const TICKETS = @json($ticketTypes);
            const EVENT_START = @json(optional($event->event_date_start)->format('Y-m-d'));
            const EVENT_END   = @json(optional($event->event_date_end)->format('Y-m-d'));
    ...
    const isPastToday = (calY < TODAY_Y) || (calY === TODAY_Y && calM < TODAY_M) || (calY === TODAY_Y && calM === TODAY_M && d < TODAY_D);
    const isOutsideEventRange = (EVENT_START && ds < EVENT_START) || (EVENT_END && ds > EVENT_END);
    const isPast = isPastToday || isOutsideEventRange;
...
async function pickDate(ds, dayNum) {
  ...
  try {
        // ✅ PERBAIKAN: Fetch SEMUA sesi sekaligus untuk tanggal ini, sertakan slug event
        const res = await fetch(`/booking/seats?tanggal=${ds}&event={{ $event->slug }}`);
...
function pickSess(id, timeText) {
    ...
    const domesticSub = (qty['dewasa'] * 85000) + (qty['anak'] * 60000);
    const domesticQ   = qty['dewasa'] + qty['anak'];
...
function buildWaDeepLink(waUrl) {
    ...
    if (selectedPaymentMethod === 'qris_static' && !uploadedBuktiUrl) {
        showToast('⚠ Upload bukti pembayaran QRIS terlebih dahulu', 'err');
        return;
    }
   ...
   const sessionObj = currentSessions.find(s => s.id === selSess);
    const sessWaktu  = sessionObj ? sessionObj.time : selSess;
    ...
    // Rincian tiket generik — kunci sesuai jenis tiket event yang aktif
    const ticketBreakdown = TICKETS
        .filter(t => qty[t.id] > 0)
        .map(t => ({ key: t.id, label: t.name, qty: qty[t.id], price: t.price }));
    ...
    const payload = {
        ...
        bukti_transfer_url: uploadedBuktiUrl,
    ...
    try {
       ...
       showBookingSuccess(deepLink, selectedPaymentMethod);
        ...
        const seatsRes = await fetch(`/booking/seats?tanggal=${selDate}&event={{ $event->slug }}`);
...
function showBookingSuccess(waUrl, paymentMethod) {
    ...
    const isQris = paymentMethod === 'qris_static';

    // ── State 1: spinner "memeriksa" (khusus QRIS statis) ──
    const checkingHtml = `
        <div style="background:#fff;border-radius:20px;padding:2.4rem 2.2rem;
                    max-width:360px;width:100%;text-align:center;
                    box-shadow:0 24px 60px rgba(26,20,69,.2);
                    animation:slideUp .3s cubic-bezier(.16,1,.3,1)">
            <div style="width:56px;height:56px;margin:0 auto 1.2rem;
                        border:4px solid rgba(26,20,69,.1);
                        border-top-color:#1a1445;border-radius:50%;
                        animation:spin .8s linear infinite"></div>
            <div style="font-size:15px;font-weight:800;color:#1a1445;margin-bottom:6px">
                Memeriksa bukti pembayaran...
            </div>
            <div style="font-size:12.5px;color:rgba(26,20,69,.55);line-height:1.6">
                Mohon tunggu sebentar
            </div>
        </div>`;

    // ── State 2: sukses + tombol WA ──
    const successHtml = `
        <div style="background:#fff;border-radius:20px;padding:2rem 2.2rem;
            ...
            <div style="font-size:13px;color:rgba(26,20,69,.55);line-height:1.6;margin-bottom:${isQris ? '10px' : '1.5rem'}">
                Data kamu sudah kami catat.<br>Tap tombol di bawah untuk konfirmasi via WhatsApp.
            </div>
            ${isQris ? `
            <div style="background:#FFF8E8;border:1px solid rgba(196,164,124,.4);
                        border-radius:10px;padding:10px 12px;margin-bottom:1.2rem;
                        font-size:11.5px;color:#8A6830;line-height:1.5;text-align:left;
                        display:flex;gap:8px;align-items:flex-start">
                <svg width="15" height="15" fill="none" stroke="#c4a47c" stroke-width="2"
                     viewBox="0 0 24 24" style="flex-shrink:0;margin-top:1px">
                    <circle cx="12" cy="12" r="10"/><path d="M12 16v-4M12 8h.01"/>
                </svg>
                <span>Bukti bayar sudah kami terima. Admin akan memverifikasi pembayaranmu — konfirmasi via WhatsApp untuk mempercepat proses.</span>
            </div>` : ''}
    ...
    notif.innerHTML = isQris ? checkingHtml : successHtml;
    ...
    // Khusus QRIS statis: tampilkan spinner dulu, baru morph ke tombol WA
    if (isQris) {
        setTimeout(() => {
            if (!document.getElementById('notif-sukses')) return; // user sempat nutup
            notif.innerHTML = successHtml;
        }, 1400);
    }
}
